...
export default function ChatSidebar({
  ...
  const bookmarked = useMemo(
    () => sessions.filter((s) => s.bookmarked && s.entries.length > 0).sort((a, b) => b.updatedAt - a.updatedAt),
    [sessions]
  );
  ...
  return (
    ...
    >
      ...
      <button
        onClick={() => router.push('/')}
        className="flex items-center gap-2.5 px-4 pt-4 pb-3 w-full text-left rounded-lg transition-opacity hover:opacity-70"
        title="시작 페이지로 이동"
      >
        <div
        ...
        {ALL_PERSONAS.map((pid) => {
          ...
          return (
            <button
              key={pid}
              onClick={() => onChangePersona(pid)}
              className="flex w-full items-center gap-2.5 rounded-lg px-2 py-1.5 text-xs font-medium transition-colors text-left"
              style={{
                background: isActive ? `${meta.colorVar}15` : undefined,
                color: isActive ? meta.colorVar : 'var(--color-text-secondary)',
              }}
              onMouseEnter={(e) => {
            ...
            >
              <span
                ...
                style={{
                  background: isActive ? meta.colorVar : 'transparent',
                  color: isActive ? '#fff' : meta.colorVar,
                  border: isActive ? 'none' : `1px solid ${meta.colorVar}40`,
                }}
              >
                {meta.icon(12)}
              </span>
      ...
      <div className="flex-1 overflow-y-auto px-2">
        ...
        {bookmarked.length > 0 && (
          <div className="mb-2">
            <p
              className="flex items-center gap-1 px-2 py-1 text-[10px] font-semibold uppercase tracking-wider"
              style={{ color: '#eab308' }}
            >
              <Star size={11} />
              즐겨찾기
